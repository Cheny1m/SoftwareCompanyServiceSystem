...
//        if ("/v1.1/login".equals(request.getRequestURI()))//||"/v1.1/login1/captcha".equals(request.getRequestURI())
//        {
//            logger.debug("登录(验证码)接口不拦截");
//            request.setAttribute(FILTER_APPLIED,true);
//            chain.doFilter(request, response);
//        }
        //获取请求头
        ...
        if (StringUtils.isBlank(header) || !header.startsWith(SecurityConstant.TOKEN_SPLIT)|| "Bearer null".equals(header)) {
            if (logger.isDebugEnabled()){
        ...
        try {
            ...
            ResponseUtil.out(response, ResUtil.getJsonStr(1000, e.getMessage()));
            return;
        } catch (Exception e){
...
//            }
            ...
            if (StringUtils.isBlank(oldToken)){
                throw new CommonException(1000, "Token被注销，请重新登录");
            }
            if(!oldToken.equals(token.replace(SecurityConstant.TOKEN_SPLIT, ""))) {
                throw new CommonException(1000, "Token已失效，请重新登录");
            }
