...
public class MetaOperationServiceImpl implements MetaOperationService {
    ...
    @Override
    ////@Cacheable(value = "operationlist",key = "#root.methodName.toString()",unless = "#result==null")
    ...
    /**
     * 查询指定url的一级导航栏,注意此方法仅限于查询一级导航栏
     * @param url
     * @return
     */
    @Override
    public Metaoperation selectByUrl(String url){
        MetaoperationExample operationExample = new MetaoperationExample();
        MetaoperationExample.Criteria criteria = operationExample.createCriteria();
        criteria.andMolurlEqualTo(url);
        List<Metaoperation> operations = metaoperationMapper.selectByExample(operationExample);
        return operations.isEmpty()?null:operations.get(0);
    }

    @Override
    @Cacheable(value = "navigationbar",key = "#root.methodName.toString().toString()",unless = "#result==null")
    ...
    @Override
    //@Cacheable(value = "navigationbar",key = "#headname+'--'+#root.methodName.toString().toString()",unless = "#result==null")
...
//        headname = headname.toLowerCase();
    ...
    @Override
    @Cacheable(value = "navigationbar",key = "#role.rid+'--'+#root.methodName.toString().toString()+#privGroup.pgid.toString()")
    public List<PrivGradationalDto> selectPrivFourLeverByRoleAndPrivgroup(Role role, Prigroup privGroup) {

        List<PrivGradationalDto> privGradationalDtos1 = rolePrivService.selectErJiBiaoTiChildrenByRole(role.getRid(), privGroup.getPrigroupdesc());
        if (privGradationalDtos1 !=null&&!privGradationalDtos1.isEmpty()){
            for (PrivGradationalDto privGradationalDto : privGradationalDtos1) {
        ...
        logger.debug("角色为:"+role.getRname()+"权限组为:"+privGroup.getPrigroupname()+"下的所有子权限为:"+ privGradationalDtos1);

        return privGradationalDtos1;
    }
    ...
    @Override
    ////@Cacheable(value = "operationlist",key = "#privgoupdesc+'--'+#root.methodName.toString().toString()")
